...
function drawXAxisTickLabels(min, max, layout, mapFunction, decimalPlaces) {
  ...
  for (i = 0; i <= layout.numXTickLabels; i++) {
    ...
    text(value.toFixed(decimalPlaces),
         layout.leftMargin + x,
         layout.topMargin - layout.pad);
    if (layout.grid) {
    // Add grid line.
    stroke(220);
    line(layout.leftMargin + x,
         layout.topMargin,
         layout.leftMargin + x,
         layout.bottomMargin);
    }    
...
function drawBottomXAxisTickLabels(min, max, layout, mapFunction, decimalPlaces) {
  // Map function must be passed with .bind(this).
  var range = max - min;
  var xTickStep = range / layout.numXTickLabels;
